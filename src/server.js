// Shared MCP server definition (tool registration). Reused by stdio (index.js) and remote HTTP (http.js).
...
export function createServer() {
  ...
  server.registerTool('encode_epc', {
  ...
  server.registerTool('generate_barcode', {
  ...
  server.registerTool('generate_qr', {
  ...
  server.registerTool('bulk_barcode', {
  ...
  server.registerTool('decode_barcode', {
    ...
    inputSchema: {
      ...
      formats: z.array(z.string()).optional().describe('Optional list of zxing formats to restrict to (e.g. ["QR_CODE","CODE_128"])')
    }
